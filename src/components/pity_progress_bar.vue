...
<template>
  <div class="pity-progress-bar-wrapper">
    ...
    <div
      v-if="mode === 'mobile'"
      class="d-flex d-lg-none align-items-center justify-content-center gap-2 py-1 bg-white border rounded-pill px-3 shadow-sm"
      style="
        font-size: 0.85rem;
        height: 32px;
    ...
    >
      ...
      >
        ...
        />
        <span class="fw-bold" :style="{ color: banner.color }"
          >{{ getPercent(banner.pity, banner.max) }}%</span
        >
      </div>
    </div>
    ...
    >
      <div
        v-for="banner in banners"
        :key="'desk-' + banner.id"
        class="d-flex align-items-center gap-1 py-0 px-2 border rounded-pill bg-light-subtle shadow-sm"
        style="
          border-color: rgba(0, 0, 0, 0.08);
          font-size: 0.75rem;
          height: 26px;
        "
        :title="banner.name + ' Pity: ' + banner.pity + '/90'"
      >
        <!-- Icon -->
        <img
          :src="banner.icon"
        ...
        >
          <div
            class="progress-bar pity_progress_bar_progress-bar progress-bar-striped progress-bar-animated"
            role="progressbar"
            :style="{
              width: getPercent(banner.pity, banner.max) + '%',
              backgroundColor: banner.color,
            }"
            :aria-valuenow="banner.pity"
            aria-valuemin="0"
            aria-valuemax="90"
          ></div>
        </div>
