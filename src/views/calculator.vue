...
const charResources = [
  { name: "Hero's Wit", exp: 20000, label: "Hero's Wit (20k)" },
  {
    ...
    exp: 5000,
    label: "Adventurer's Experience (5k)",
  },
...
const weaponResources = [
  {
    ...
    exp: 10000,
    label: "Mystic Enhancement Ore (10k)",
  },
  {
    name: "Fine Enhancement Ore",
    exp: 2000,
    label: "Fine Enhancement Ore (2k)",
  },
  { name: "Enhancement Ore", exp: 400, label: "Enhancement Ore (400)" },
];

const activeResources = computed(() =>
  calcType.value === "character" ? charResources : weaponResources,
);
...
const calculateResinLogic = () => {
  ...
  const finishTimeString = finishedAt.toLocaleTimeString("en-US", {
    hour12: false,
  });
...
const calculateExpLogic = () => {
  ...
  if (calcType.value === "character") {
    targetExp =
      character_exp[intendedLevel.value - 1] -
      (character_exp[currentLevel.value - 1] + currentExp.value);
  } else {
    ...
    targetExp =
      weapon_exp[curveIndex][intendedLevel.value - 1] -
      (weapon_exp[curveIndex][currentLevel.value - 1] + currentExp.value);
  }
  ...
  moraNeeded.value =
    calcType.value === "character" ? totalExpGained / 5 : totalExpGained / 10;
...
const addTodo = () => {
  ...
  const formattedItems = activeResources.value
    .map((res, index) => {
      return {
        name: res.name,
        label: res.label,
...
<template>
  <div
    class="container-fluid d-flex flex-column justify-content-center mt-5 mb-5"
  >
      ...
      <div class="col-12 col-md-8 col-lg-6">
        <div class="card custom-dark-card p-4 mb-4">
          <div class="btn-group w-100 mb-4 shadow-sm">
            <button
              class="btn btn-nav"
              :class="{ active: calcType === 'character' }"
              @click="changeType('character')"
            >
              Character
            </button>
            <button
              class="btn btn-nav"
              :class="{ active: calcType === 'weapon' }"
              @click="changeType('weapon')"
            >
              Weapon
            </button>
            <button
              class="btn btn-nav"
              :class="{ active: calcType === 'resin' }"
              @click="changeType('resin')"
            >
              Resin
            </button>
          </div>
          ...
          <div v-if="calcType === 'character' || calcType === 'weapon'">
            <div
              v-if="gachaStore.is_fetching_data"
              class="text-dark mb-3 text-center fw-bold"
            >
              Loading database...
            </div>
            ...
            <div v-else>
              ...
              <div class="mb-3" v-if="calcType === 'character'">
                ...
                <select
                  class="form-select text-dark custom-input"
                  v-model="selectedCharacter"
                >
                  <option
                    v-for="char in gachaStore.character_list"
                    :key="char.name"
                    :value="char"
                  >
              ...
              <div class="mb-3" v-if="calcType === 'weapon'">
                ...
                >
                  <option
                    v-for="weap in gachaStore.weapon_list"
                    :key="weap.name"
                    :value="weap"
                  >
          ...
          <div v-if="calcType === 'resin'">
            <div class="mb-3">
              <label class="text-dark small mb-1 text-center w-100"
                >Current Resin</label
              >
              <input
                type="number"
                class="form-control custom-input py-2 text-dark"
                v-model.number="currentResin"
                min="0"
                max="2000"
              />
            </div>
          ...
          <button
            ...
            @click="calculate"
            :disabled="
              gachaStore.is_fetching_data ||
              (!selectedCharacter && calcType === 'character')
            "
          >
        ...
        <!-- EXP Results Card -->
        <div
          class="card custom-dark-card p-4"
          v-if="
            hasCalculated && (calcType === 'character' || calcType === 'weapon')
          "
        >
          ...
          >
            <li
              v-for="(item, index) in activeResources"
              :key="index"
              class="list-group-item d-flex justify-content-between align-items-center bg-light-blue text-dark border-secondary py-3"
            >
              {{ item.label }}
          ...
          <div
            class="info-box p-3 mb-2 rounded bg-light-blue d-flex justify-content-between align-items-center"
          >
            <span class="text-black">Mora Needed</span>
            <span class="text-dark fw-bold fs-5">{{
              Math.ceil(moraNeeded).toLocaleString()
            }}</span>
          </div>
          <div
            class="info-box p-3 rounded bg-light-blue d-flex justify-content-between align-items-center"
            v-if="wastedExp > 0"
          >
            <span class="text-black">Wasted EXP</span>
            <span class="text-danger fw-bold fs-5">{{
              wastedExp.toLocaleString()
            }}</span>
          </div>
          <button
            class="btn btn-outline-primary w-100 mt-3 py-2 fw-bold"
            @click="addTodo"
          >
        ...
        <div
          class="card custom-dark-card p-4"
          v-if="hasCalculated && calcType === 'resin'"
        >
          <div class="resin-results p-4">
            ...
            <div
              class="text-danger mt-3 pb-2 border-bottom border-secondary fw-bold"
            >
