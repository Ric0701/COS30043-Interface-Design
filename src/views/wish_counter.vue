  ...
  <div class="container-fluid d-flex min-vh-100 mt-5 mb-5 pt-5 flex-column">
    ...
    <div v-else>
      <div class="d-flex align-items-center mb-4">
        <button
          class="btn btn-link text-dark text-decoration-none fs-3 me-2"
          @click="activeHistory = ''"
        >
          &larr;
        </button>
        <h2 class="fw-bold mb-0 text-dark">
          Wish Counter <span class="text-primary">{{ historyTitle }}</span>
        </h2>
      </div>
        ...
        <div class="col-12">
          <div class="card custom-dark-card p-3">
            <div class="table-responsive">
              <table
                class="table table-dark-blue table-hover mb-0"
              ...
              >
                ...
                <tbody>
                  <tr
                    v-for="(pull, index) in gacha_store[
                      `${activeHistory}_lifetime_pulls`
                    ]
                      .slice()
                      .reverse()"
                    :key="index"
                  >
                    <td class="text-muted">
                      {{ pull.time ? pull.time.replace("T", " ") : "N/A" }}
                    </td>
                    ...
                    <td class="text-muted">
                      {{
                        gacha_store[`${activeHistory}_lifetime_pulls`].length -
                        index
                      }}
                    </td>
                  </tr>
